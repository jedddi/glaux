...
export function generateStubFailureSamples(evaluation: Evaluation): StubFailureSample[] {
  ...
  if (labels.length === 0) {
    return generateFallbackSamples(rand)
  }
  ...
  const samples: StubFailureSample[] = []
  let rank = 1
...
function generateFallbackSamples(rand: () => number): StubFailureSample[] {

...
import type { FailureAnalysisRunner } from "@/lib/execution/adapter"
import { stubFailureAnalysisRunner } from "@/lib/execution/stub-failure-runner"
...
export async function runFailureAnalysis(
  ...
  runner: FailureAnalysisRunner = stubFailureAnalysisRunner
  ...
  try {
    assertValidTransition("queued", "running", "failure-analysis")
    await updateFailureAnalysisStatus(analysis.id!, "running", {
      started_at: new Date().toISOString(),
    })

    const samples = await runner.analyze(evaluation)

    const dbSamples = samples.map((s) => ({
    ...
    const topConfusedPairs = computeTopConfusedPairs(samples)
    const failureByTrueClass = computeClassDistribution(samples, "actual_label")
    const failureByPredictedClass = computeClassDistribution(samples, "predicted_label")
